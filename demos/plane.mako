...
rhs = @urlParams.get 'rhs', 'float', 1
rhsstr = if rhs != 0 then rhs else ''
...
new Demo {}, () ->
    ...
    if plainPlane
        ...
        updateCaption = () =>
            katex.render """(x,\\,y,\\,z)
                              = (#{rhsstr}-#{coeffs[0]}-#{coeffs[1]},\\,#{coeffs[0]},\\,#{coeffs[1]})
                              = \\color{#{pointColor.str()}}{({#{(rhs-cf(0)-cf(1)).toFixed(2)}},\\,
                                 {#{cf(0).toFixed(2)}},\\,
                                 {#{cf(1).toFixed(2)}})}
                         """, @vecElt
    ...
    clipCube = @clipCube view,
        draw:   true
        hilite: false
    trans = clipCube.clipped.transform position: [rhs, 0, 0]
