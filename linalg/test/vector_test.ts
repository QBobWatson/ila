...
const vec = (...entries: number[]): Vector => new Vector(...entries);


describe('Vector', () => {
    describe('#from()', () => {
        it('should create a vector with a map function', () =>
            Vector.from([1,2,3], x => x*2).should.eql(vec(2,4,6)));
        it('should use the identity by default', () =>
            Vector.from([1,2,3]).should.eql(vec(1,2,3)));
    });
    describe('#constant()', () => {
        it('should create a constant vector', () =>
            Vector.constant(3, 1).should.eql(vec(1, 1, 1)));
        it('should create vectors of size one correctly', () =>
    ...
    describe('#sizesq()', () =>
        it('should have the correct squared size', () =>
            vec(1, 2, 3).lensq.should.equal(1 + 4 + 9)));
    describe('#size()', () =>
        it('should have the correct size', () =>
            vec(3, 4).len.should.be.approximately(5, 1e-10)));
    describe('#normalize()', () => {
    ...
    describe('#add() and #sub()', () => {
        ...
        it('should throw when vectors have different sizes', () => {
            let v = vec(3, 4);
            v.add.bind(v, vec(1, 2, 3)).should.throw(/different sizes/);
        });
    });
    describe('#scale()', () =>
    ...
    describe('#dot()', () => {
        ...
        it('should throw when vectors have different sizes', () => {
            let v = vec(3, 4);
            v.dot.bind(v, vec(1, 2, 3)).should.throw(/different sizes/);
        });
    });
});
